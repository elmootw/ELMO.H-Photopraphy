...
1. **新增相簿（含 Hashtag 篩選）**
   ```bash
   # 資料夾名稱格式：[tag1][tag2] 相簿名稱
   mkdir -p "public/images/wedding/[登記] 王小明婚禮"
   mkdir -p "public/images/lifestyle/[抓周][團拍] 活動名稱"
   # 將照片放入，確保有 cover.jpg
   cp cover.jpg photo-*.jpg "public/images/wedding/[登記] 王小明婚禮/"
   ```
...
支持分類：`lifestyle`、`wedding` 及其他相簿結構分類

**資料夾命名約定：**
- 使用 `[` 與 `]` 包裹的文字為 Hashtag 標籤
- 多個標籤以空白分隔
- 相簿名稱緊跟其後，並以空白分隔
...
**功能說明：**
- 系統會自動讀取資料夾名稱中的 Hashtag 標籤
- 在相簿詳情頁面提供按 Hashtag 篩選功能
- 支持多個標籤的 AND/OR 篩選
